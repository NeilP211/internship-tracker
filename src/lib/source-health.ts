...
// Shared by the /api/internships/source-health route (live mode) and
// scripts/export-static.ts (GitHub Pages snapshot). Lives outside the route
// file because the static build stashes src/app/api/ away entirely.
...
export async function buildSourceHealth(): Promise<{ sources: SourceHealthEntry[] }> {
  ...
  type Entry = Omit<SourceHealthEntry, "name">;
  ...
  const sources = Array.from(sourceMap.entries())
    ...
    // that is neither polled this cycle nor contributed a row in the last 7
    // days: historical-only rows should not read as a live, down source.
